...
type PreorderStatus = 'active' | 'closed' | 'coming_soon';
...
const STATUS_LABELS: Record<PreorderStatus, { label: string; color: string }> = {
  active:      { label: 'Active',      color: 'bg-gray-900 text-white' },
...
export default function AdminPreordersPage() {
  ...
  const [editingEntry, setEditingEntry] = useState<Preorder | null>(null);
  ...
  const [form, setForm] = useState({
    ...
    price: '',
    image_url: '',
    status: 'active' as PreorderStatus,
  });
  ...
  const openAdd = () => {
    ...
    setForm({ product_name: '', description: '', price: '', image_url: '', status: 'active' });
    setFormError('');
    setShowModal(true);
  };

  const openEdit = (entry: Preorder) => {
  ...
  const handleSave = async () => {
    ...
    const { error } = editingEntry
      ? await supabase.from('preorders').update(payload).eq('id', editingEntry.id)
      : await supabase.from('preorders').insert(payload);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Available Goods in Ghana</h1>
          <p className="text-sm text-gray-500 mt-0.5">
            Manage goods available for purchase in Ghana
          </p>
        </div>
        <button
          onClick={openAdd}
      ...
      <div className="grid grid-cols-3 gap-4">
        {([
          { label: 'Active', count: counts.active, icon: 'ri-fire-line' },
          { label: 'Coming Soon', count: counts.coming_soon, icon: 'ri-time-line' },
          { label: 'Closed', count: counts.closed, icon: 'ri-close-circle-line' },
        ] as const).map((stat) => (
          <div key={stat.label} className="bg-white rounded-xl border border-gray-200 px-5 py-4">
            <div className="flex items-center gap-2 mb-1">
              <i className={`${stat.icon} text-gray-400`} />
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            <i className="ri-shopping-bag-line text-4xl text-gray-200 block mb-2" />
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 text-left">
                  ...
                  <th className="px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide hidden md:table-cell">Status</th>
                ...
                {filtered.map((entry) => (
                  <tr key={entry.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {entry.description && (
                            <p className="text-xs text-gray-400 truncate max-w-[180px]">{entry.description}</p>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-3.5 hidden md:table-cell">
                      <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold ${STATUS_LABELS[entry.status].color}`}>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg my-4">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                <select
                  value={form.status}
                  onChange={(e) => setForm((f) => ({ ...f, status: e.target.value as PreorderStatus }))}
                  className={inputClass}
                >
                  <option value="active">Active — customers can preorder</option>
                  <option value="coming_soon">Coming Soon — visible but not orderable</option>
                  <option value="closed">Closed — hidden from customers</option>
                </select>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 text-center">
            ...
            <p className="text-sm text-gray-500 mb-6">
              This will permanently remove this listing. This cannot be undone.
            </p>
